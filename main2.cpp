...
class Graph{
private:
	vector<pair<int, int>> nodeEdges[X*Y]; //index, weight
	...
	int dtsValue(int index, vector<int> path, int startIndex){
		if(index == startIndex)
			return 0;
		return nodeEdges[index].second + dtsValue(path[index], path, startIndex);
	}
public:
	Graph(int array[X][Y]){
		for(int i = 0; i < Y; ++i){
	...
	vector<int> findPath(int startIndex, int endIndex){
		vector<int> distanceToEnd(X*Y, INFINITY);
		vector<int> distanceToStart(X*Y, INFINITY);
		set<pair<int, int>> openSet;
		vector<int> theBestPath(X*Y, -1);
		
		distanceToStart[startIndex] = 0;
		distanceToEnd[startIndex] = hValue(startIndex, endIndex);
		...
		while(!openSet.empty()){
			...
			for(auto it : nodeEdges[current]){
				int weight = dtsValue(it.first, theBestPath, startIndex) + hValue(it.first, endIndex);
